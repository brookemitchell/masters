...
* Introduction
    Parasite Suite is collection socially-orientated art works exploring attitudes and reactions to digital technology, set in a particpatory environment. The project is realised using a set of art projects that consider the premise of the title, that we live in a world where 'computationality' defines our mode of being (otherwise known as ontotheology)[fn:40]. Following the work of David Berry, 'Computationality' can be broken into what I feel are its two defining aspects for users, that the machines encourage a networked experience and user thinking in terms of abductive aesthetics.

* Networked Experience
The manner in which human beings order and recognise the world is takena as oone that is open to influence and changes based on environmental trends and internal changes. Of those external trends I argue that how we experience the world and what are the metaphors that shape our reality have shifted since the turn of the 21st century. In a society that now has near constant engagment with software algorithms, especially in many processes that we consider as being 'away from the computer'. For instance the production and shipping of any good is now entirely dependant on real time monitoring and 'just in time' production. Berry (2011), and avid reader of Martin Heidegger, argues that in contrast to the /technnicity[fn:42]/ of electricity that ordered the /mode of revealing/ of Heidegger's world, today's engagements witht the world order themselves around the metaphor of the network of streaming data. The experience, of considering oneself as part of a network, is to order oneself in response of 'the stream', settings ones goals in terms of processing information, performing Algorithmic transformations to process the deluge and searching for ways to make information /computeable/, which leads to our second quality of computationality, abductive reasoning.
...
* EDIT As we socialise and surveil
...
The characteristics of this order and understanding of the world are based on a 'Mode of Revealing' develped around the metapohor of 'Streaming-forth'[fn:2].
