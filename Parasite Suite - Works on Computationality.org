...
** Technical Outline
   ...
   The installation consists of a open top glass box of dimensions 0.75 meter x 1m x 0.15 meter depth, filled with 50 kilograms of white sand. Directly above the box a short-throw projector and depth sensing camera (Microsoft Kinect) are mounted. These are connected to a desktop computer running Linux with a graphics card and audio output. Management of the camera input data and visual rendering software is controlled by the software SARndbox, an augmented virtual reality system developed by Oliver Kreylos at the  University of Davis California Geology Department.[fn:56] The software forms a feedback loop as the calibrated depth information from the camera becomes topographical data, which can be adjusted in real time by users altering the depth of the sand surface. I have created a custom version of the SARndbox software that adds the features of Open Sound Control(OSC) to the software. From this the depth matrix of the sand surface is constantly transmitted over a user selected port to be used by the IanniX[fn:57] three dimensional OSC sequencer. IanniX takes the depth matrix and transforms this into a set of curves

 to be passed to synthesis intruments built in Overtone[fn:66], a sortware which uses the SuperCollider sythesis engine.[fn:69]
