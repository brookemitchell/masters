...
*** p 1
	 ...
	 Parasite Suite is a artistic investigation of social possibilities for  engagements with digital technology. The project is realised by using sonicly orientated art projects to meditate on the theme of 'computationality'. 'Computationality' is a term I define to mean the combination of compputational processing and networking that reifies itself as both an aesthetic and mode of experience for those that interact with a machine [fn:1]. As the key function of the works is in creating scenarios in which the role of participants can dramatically change, for this project it made sense to talk only of these actors as participants. The projects are open ended and generative, with lines between artist and audience blurred.
...
** How we got to where we are today														 :Heading:Edit:
