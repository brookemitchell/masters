...
*** EDIT The Social and the Surveillant
     ...
     Parasite Suite is collection socially-orientated art works exploring attitudes and reactions to digital technology under a particpatory setting. The project is realised using a set of art projects that meditate on the premise that we live in a world where 'computationality' defines our mode of being. Computationality can be defined as a ontotheology, based within a specific historical 'epoch' (which we appear to be in at present) defined by practices, methods and categories based on an understanding that computers form part of our "essence and existence," as a set of knowledges, practices, methods and categories that "temporarily secures the intelligible order[fn:1]"
*************** Go into Heidegger
*************** END
